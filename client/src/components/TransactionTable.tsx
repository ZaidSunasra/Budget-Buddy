...
export function TransactionTable({ data, onDelete }: { data: any, onDelete: () => void}) {
                ...
                {data && data.expenses.map((expense: expenseInput) => (
                    <TableRow key={expense.id} className="font-bold text-md">
                        ...
                        <TableCell className="flex gap-5">
                            ...
                            <Button className="bg-secondary text-red-500" onClick={() => setIsOpen(true)}> <Trash2 /> </Button>
                            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                                <DialogContent className="sm:max-w-md">
                                    ...
                                    <DialogFooter className="sm:justify-start">
                                        <Button type="submit" className="px-3 flex gap-1 items-center" variant="destructive" onClick={() => deleteTransaction(expense.id)}>
