...
export const addBudget = async (
  { allocated_value, category }: addBudgetBody,
  id: string,
): Promise<void> => {
  await db.query(
    "INSERT INTO user_budgets(category, allocated_value, user_id) VALUES($1, $2, $3)",
    [category, allocated_value, id],
  );
};

export const editBudget = async (
  { allocated_value }: editBudgetBody,
  id: string,
): Promise<void> => {
  await db.query("UPDATE user_budgets SET allocated_value=$1 WHERE id=$2", [
    allocated_value,
    id,
  ]);
};
